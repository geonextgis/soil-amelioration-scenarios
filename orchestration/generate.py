...
def write_campaign_script(path: Path, listing: Path, repo: Path, cfg: dict,
    ...
    The cost of that is one large allocation: it starts only when the whole node
    set is free at once, and it must be given a walltime that covers every
    experiment end to end. Anything not reached before the walltime expires is
    left for a resumed submission (SIMPLACE_RESUME=1), which is why each
    experiment writes a completion marker.
    """
    s = cfg["slurm"]
    ...
    # Steps are sized to the allocation, not to num_nodes: memory has to be
    # requested per CPU, or the first srun step on a node claims all of it and
    # the other tasks-per-node steps queue behind it inside our own allocation.
...
while read -r crop cfgfile <&3; do
  ...
  python "simplace/$crop/simplace_runner_cluster.py" "$cfgfile" --mode alloc \\
...
xargs -P {par} -L1 -a "{listing}" \\
  sh -c 'python simplace/"$0"/simplace_runner_cluster.py "$1" --mode sbatch'
""")
        script.chmod(0o755)

        # Preferred path: hold the nodes for the whole campaign instead of
        # re-queueing between experiments.
        ...
        for s in candidates:
            if s in (script, campaign):
                continue
            # The manifest is the script's own stem for submit_*.sh, but a
            # campaign script points at the submit_* list it shares, so fall
            # back to whatever .list the script names before giving up and
            # scraping inline config paths (the pre-manifest script format).
            text = s.read_text()
