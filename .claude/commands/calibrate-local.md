...
argument-hint: <crop> <phenology|lai|yield> [--iterations N] [--locations N]
---
...
Arguments: **$ARGUMENTS** (defaults: crop `winter_wheat`, target `lai`).
...
3. **Run the loop**:
   ...
   is scored and appended to the same ledger `/calibrate-lai` writes to.
...
- Order is phenology (validate only) → LAI → `calibrate.py handoff` → yield.
- Phenology is protected: `--target phenology` validates. Recalibration needs
  `--mode recalibrate` and should only be used when the user asks for it.
